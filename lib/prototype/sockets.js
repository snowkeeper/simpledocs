...
module.exports = function(UserIO) {
	...
	this.socket.on("connection", function(socket) {
		...
		/* add build routes */
		socket.on(route  + ':buildPages', function() {
			//console.log('Build Pages');
			//simpledocs.gitHubPages.build();
		});
		
		/* add publish routes */
		socket.on(route  + ':publishPages', function() {
			//simpledocs.gitHubPages.publish();
		});
