...
module.exports = function(keystone, callback, config) {
	...
	keystone.init({
		...
		'admin path': config['admin path'] || simpledocs.get('admin path') || 'keystone',
		...
		'signin redirect': config.redirect || simpledocs.get('signin redirect') ||  page,
		'signout redirect': config.signoutRedirect || simpledocs.get('signout redirect') ||  page,
	...
	if(simpledocs.get('allow register') === true) {
		greeter.init({
			keystone: keystone,
			debug: true,
			'new user can admin': true,
		})
		keystone.set('signout redirect', greeter.get('route greeter') || keystone.get('signout redirect'));
	}
	...
	keystone.start({
		...
		onStart: function() {
			...
			simpledocs.emit('keystone start');
		}
	});
}
/**
