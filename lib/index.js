...
SimpleDocs.prototype.add = function(callback) {
	...
	app.get(view+"/simpledocs/config",function(req,res) {
		
		//send a javascript header and file contents
		res.set('Content-Type', 'text/javascript');
		var snowdone = function() {
			var config = {singleBranch:simpledocs.get('menu single branch'),collapse: simpledocs.get('menu collapse'),name: modelConfig.label,homepage: simpledocs.get('homepage'),api:{page: api+'/page',tree: api+'/tree'},path:{root:view,logout:keystone.get('signout url')},isMe: keystone.security.csrf.getToken(req, res),isKey: keystone.security.csrf.getToken(req, res),breaks: {small:{width:768}},themeToToggle:simpledocs.get('theme') || 'dark-theme'};
