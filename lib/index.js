...
SimpleDocs.prototype._keystone = function() {
	...
	if(standalone) {
		...
		start.custom.call(this,app,mongoose,function(){_this.emit('model');});
		...
		if(doc.sortBy === 1 ) {
			...
			keystone.list(model).model.count({parent:doc.parent},function(err,count) {
				keystone.list(model).model.findOne({"_id":parent}).select('order title').exec(function(err,point) {
					...
					if(doc.sortBy === 3 ) {
						/* this is a before, set doc.order  to point.order-1 unless it is the first item then set to 1*/
						if(point.order === 1) {
