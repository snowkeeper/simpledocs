...
SimpleDocs.prototype.model = function() {
	...
	SimpleDocModel.add({
		...
		sortBy: {type: Types.Select, numeric: true, options: [{ value: 1, label: 'Current Order if defined' },{ value: 2, label: 'Enter Number' }, { value: 3, label: 'Before Document' }, { value: 4, label: 'After Document' }], label: 'Order Placement', emptyOption: false },
		...
		order: {type: Types.Number, noedit:false, index:true,note:'only change this value if you need to manully set the order.  Try and let Order Placement order for you'},	
	});
	...
	*/
	SimpleDocModel.schema.statics.getPage = function getPage(slug,cb) {
		...
		model.findOne({slug:slug}).select('-__v').populate('parent links').lean().exec(function(err,doc) {
	...
	*/
	SimpleDocModel.schema.pre('save', function(next) {
		...
		var inc = function(doc,cb) {
			...
			var plus = doc.order;
			var minus = doc.order;
			keystone.list(model).model.find({ "parent" : doc.parent ,_id:{$ne:doc._id}}).sort("order").exec(function(err,coll){
				if(err){
				...
				coll.forEach(function(v,k) {
					if(v.order <= doc.order && doc.order>1) {
						l.push(v);
						
					} else  {
						plus++;
						var i = plus;
						keystone.list(model).model.update({ _id : v._id},{order:i,sortNumber:i}).exec(function(err){});
						console.log('update plus order ',i,v.title)
					}
					
				});
				l.reverse();
				l.forEach(function(vv,kk) {
					minus--;
					var i = minus;
					keystone.list(model).model.update({ _id : vv._id},{order:i,sortNumber:i}).exec(function(err){});
					console.log('update minus order ',i,vv.title)
				});
		...
		if(doc.sortBy === 1) {
			...
			keystone.list(model).model.findOne({"_id":parent}).select('order').exec(function(err,point) {
				if(err)return finish(err);
				console.log('get parent sort order',parent,point,err);
				if(!point || point === null) return finish();
				
				
				doc.order = doc.sortBy === 3 ? point.order: point.order + 1;
			...
			keystone.list(model).model.count({parent:doc.parent},function(err,count) {
				console.log('sort',sort,'sortN',sortN,'count',count);
				if(count === 0 && parent != null && parent != undefined) {
					doc.order = 1;	
				}
				if(sort	=== 'prepend' || (sortN === 0 && sort === '0')) {
					doc.order = parent != null && parent != undefined ? 1 : 0;
