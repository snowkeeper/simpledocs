...
SimpleDocs.prototype.start = function(userSuppliedKeystone, config) {
	/* global keystone */
	if(userSuppliedKeystone) {
		keystone = userSuppliedKeystone;
		Types = keystone.Field.Types;
	}
 ...
 * @api public
 */ 
SimpleDocs.prototype.sockets = function(io) {
	
	if(!io) {
		Live.init(keystone).apiSockets();
		io = Live.io;
	}
...
SimpleDocs.prototype._keystone = function() {
	...
	if(standalone) {
		...
		standalone.call(this,keystone,function(){_this.emit('model',keystone);});
		
	} else if(app && mongoose) {
		var custome =  require('./start/custom.js');
		custome.call(this,app,mongoose,function(){_this.emit('model',keystone);});
		
	} else {
		this.emit('model',keystone);
	}
...
SimpleDocs.prototype._routes = function() {
	...
	var _this = this;
	/* add our models */
	this.add(function(){_this.emit('complete',keystone)});	
...
SimpleDocs.prototype.statics = function() {
	...
	var materialView = this.get('material route');
	var SD = this;
	/* we use snowhub in the UI so we dont clash anywhere
	 * */
	app.use("/snowdocs", express.static(SD.get('moduleDir') + "/public"));
	/* material ui */
	app.use('/simpledocs-files', express.static(path.join(SD.get('moduleDir'), 'app')));
	app.use(
		'/simpledocs-css',
		less(
	...
	app.get(materialView, materialRoute);
	app.get(materialView + '/*', materialRoute);
	
	function materialRoute( req, res ) {
		var options = {
...
SimpleDocs.prototype.add = function(callback) {
	 ...
	 * */
	var 	app = keystone.app,
		view = this.get('route') || '/simpledocs',
		materialView = this.get('material route') || '/m-ui',
		setview = view,
		simpledocs = this,
	...
	var publicRoute = function(req, res) {
			...
			var locals = {
				...
				pathConfig: view+"/simpledocs/config",
				apiTree: api+'/tree',
				apiPage: api+'/page',
	...
	app.get(view+"/simpledocs/config", sdConfig); 
	app.get("/simpledocsconfig", sdConfig);
