...
SimpleDocs.prototype.model = function() {
	...
	SimpleDocModel.add({
		...
		currentOrder: {type: Type.Number, noedit:true, label:'Current Order' },
		...
		order: {dependsOn: { sortBy: 5 }, type: Types.Number, noedit:false, index:true,note:'only change this value if you need to manully set the order.  Try and let Order Placement order for you'},	
	});
			...
			keystone.list(model).model.find({ "parent" : doc.parent ,_id:{$ne:doc._id}}).sort("order").exec(function(err,coll){
				...
				coll.forEach(function(v,k) {
					if(v.order <= doc.order && doc.order>1) {
						...
						plus++;
						var i = plus;
						keystone.list(model).model.update({ _id : v._id},{order:i,currentOrder:i}).exec(function(err){});
				...
				l.forEach(function(vv,kk) {
					minus--;
					var i = minus;
					keystone.list(model).model.update({ _id : vv._id},{order:i,currentOrder:i}).exec(function(err){});
