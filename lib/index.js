...
var SimpleDocs = function(config) {
	...
	/* build options */
	this.set('build path', this.get('moduleDir') + '_build/');
	this.set('build path ghpages', this.get('build path') + 'gh-pages/');
	this.set('build path tmp', this.get('build path') + 'tmp/');
