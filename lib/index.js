		...
		var inc = function(doc,cb) {
			...
			keystone.list(model).model.find({ "parent" : doc.parent }).sort("order").exec(function(err,coll){
				...
				coll.forEach(function(v,k) {
					console.log('check',v.title,v.order,i,doc.id);
					if(doc.order === i) i++;
					if(doc.key !== v.key) {
		...
		if(doc.sortBy === 1) {
			...
			keystone.list(model).model.count({parent:doc.parent},function(err,count) {
				keystone.list(model).model.findOne({"_id":parent}).select('order').exec(function(err,point) {
					...
					if(doc.sortBy === 3 && point.order === count) {
						...
						/* this is an after, set doc.order to point.order + 1 unless it is the last item then set to equal*/
						if(point.order === count) {
							doc.order = point;
						} else {
							doc.order = point + 1;
					...
					doc.order = doc.sortBy === 3 ? point.order : point.order;
