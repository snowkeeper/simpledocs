$(function() {
	...
	var socketLists = io('//snowx:48433/lists');
	...
	socketLists.on('lineup',function(data) {
		console.log('lineup',data);
		updateConsole('lineup',data)
	});
	socketLists.on('lineups',function(data) {
		console.log('lineups',data);
		updateConsole('lineups',data)
	});
	...
	$('#epgapi').click(function(){
		...
		.done(function( resp,status,xhr ) {
			updateConsole('EPG: ' + $path , url, resp);	
		});
		
	});	
	
	$('#epgsock').click(function(){
		...
		socketLists.emit(emit,{action:$epg});
